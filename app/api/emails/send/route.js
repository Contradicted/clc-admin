  ...
  try {
    ...
    for (const student of students) {
      try {
        ...
        console.log(
          `DEBUG - Student ${student.id} email fields:`,
        ...
        console.log(
          `DEBUG - Send to both emails? ${sendToBothEmails ? "Yes" : "No"}`
        );
        ...
        if (sendToBothEmails) {
          ...
          if (student.enrolledStudent?.office365Email) {
            console.log(
              `DEBUG - Adding office365Email: ${student.enrolledStudent.office365Email}`
            );
          ...
          if (student.enrolledStudent?.personalEmail) {
            console.log(
              `DEBUG - Adding personalEmail: ${student.enrolledStudent.personalEmail}`
            );
        ...
        for (const emailAddress of uniqueEmailAddresses) {
          console.log(
            `Attempting to send email via Graph API to: ${emailAddress}`
          );
          await graphClient
        ...
        console.log(
          `Emails sent for student ${student.id}: ${uniqueEmailAddresses.join(", ")}`
        );
      } catch (error) {
        const studentInfo = student
          ? `to ${student.email} (ID: ${student.id})`
          : "(unknown student)";
        ...
        try {
          if (student) {
            ...
            console.log(
              `Failed emails for student ${student.id}: ${attemptedEmails.join(", ")}`
            );
          }
        } catch (dbError) {
          console.error(
            `Failed to log email error to database for student ${student?.id}:`,
            dbError
          );
        }
      }

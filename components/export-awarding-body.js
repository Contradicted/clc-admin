...
const ExportAwardingBody = ({ data, courses }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/export-awarding-body?${params}`, {
        method: "GET",
      });
      ...
      const filename = filenameMatch
        ? filenameMatch[1]
        : `${courseTitle.replace(/[^a-zA-Z0-9]/g, "_")}_${campus}_${commencement.replace(/[^a-zA-Z0-9]/g, "_")}_awarding_body_${new Date().toISOString().split("T")[0]}.csv`;
      ...
      <div className="bg-gradient-to-r from-blue-500 to-blue-600 p-6 text-white">
        <div className="flex items-center gap-2">
          <h2 className="text-2xl font-bold">
            Awarding Body Registration Export
          </h2>
        </div>
        <p className="text-blue-100 mt-2">
          Export student registration status with awarding bodies
        </p>
      </div>
...
export default ExportAwardingBody;

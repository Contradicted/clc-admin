...
/**
 * Finds a team by its course name
 ...
 */
async function findTeamByCourseName(courseName) {
  try {
    const response = await graphClient
      .api("/groups")
      .filter(
        `resourceProvisioningOptions/any(x:x eq 'Team') and displayName eq '${courseName}'`
      )
      .select("id,displayName")
      .get();

    // Since we filtered for exact match, we just need the first result
 ...
 */
async function addStudentToTeam(studentEmail, courseName) {
  try {
    ...
    const team = await findTeamByCourseName(courseName);
 ...
 * @returns {string} Three letter campus code
 */
function getCampusCode(campus) {
  const campusCodes = {
    'London': 'LON',
    'Bristol': 'BRI',
    'Sheffield': 'SHE',
...
/**
 * Generates the Teams team name based on course, campus and commencement date
 ...
 */
function generateTeamName(courseName, commencementDate, campus) {
  const date = new Date(commencementDate);
  const month = date.toLocaleString('default', { month: 'short' }).toUpperCase().slice(0, 3);
...
export async function createStudentAccount(student) {
  ...
  try {
    ...
    if (student.course) {
      const teamName = generateTeamName(student.course, student.commencementDate, student.campus);
      await addStudentToTeam(email, teamName);
    }

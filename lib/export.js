...
export function exportApplicationToCSV(data) {
  ...
  let headers = [
    ...applicationHeaders
      .filter(
        (header) => !["Bristol Campus", "London Campus", "Sheffield Campus"].includes(header.label)
      )
...
export async function exportAllApplicationToCSV(
  ...
  try {
    const filteredCampus = selectedFields.filter(
      (sf) => sf === "field_bristol_campus" || sf === "field_london_campus" || sf === "field_sheffield_campus"
    );
    ...
    let headers =
      selectedFields && selectedFields.length > 0
        ...
        : applicationHeaders
            .map((header) => header.label)
            .filter(
              (label) => label !== "Bristol Campus" && label !== "London Campus" && label !== "Sheffield Campus"
            );
  ...
  try {
    ...
    const csvContent = [
      ...
      ...applications.map((app, index) => {
        ...
        const dob = app.dateOfBirth ? dayjs(app.dateOfBirth).format("DD-MMM-YYYY") : "";
    ...
    console.error("[EXPORT_APPLICATION_DATA_ERROR]", error);
    return { error: error.message || "Failed to export application data" };
  }
}

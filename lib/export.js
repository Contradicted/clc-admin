...
export async function exportStudentFinanceByDateRange(dateRange) {
  try {
    ...
    const groupedData = processedData.reduce((acc, item) => {
      ...
      const amount = parseFloat(item.paymentAmount.replace(/[GBP,]/g, '')) || 0;
    ...
    // Create CSV content with course grouping
    let csvRows = [];
    let grandTotal = 0;
    ...
    sortedCourses.forEach(course => {
      ...
      // Add column headers with payment order
...
export async function exportStudentData(
  ...
  try {
    ...
    // Create CSV content
    const csvContent = formatCsvContent(headers, applications.map((app, index) => {

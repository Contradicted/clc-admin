...
export async function exportStudentFinanceData(
  ...
  try {
    ...
    const rows = applications.map((app) => {
      ...
      // Only try to parse expectedPayments if paymentPlan exists and has expectedPayments
      ...
      // Format payment data for up to 3 payments
      const payments = Array(3)
        .fill(null)
        .map((_, index) => {
          const payment = expectedPayments[index] || {};
          return [
            payment.date
      ...
      return [
        ...
        toTitleCase(app.emergency_contact_name || ""),
        app.emergency_contact_no || "",
        app.paymentPlan?.crn || "", // Will be empty if no paymentPlan
        universityAndCourse,
        ...payments.flat(), // Will be empty strings if no payments

...
export async function exportAllApplicationToCSV(
  ...
  try {
    ...
    const filename = `all-applications-${now}`;
    let excludeColumns = [];
    ...
    let headers =
      selectedFields && selectedFields.length > 0
        ? selectedFields
            .map((field) =>
              applicationHeaders.find(
                (h) => h.id === field || h.value === field
              )
            )
            .filter(Boolean)
            .map((h) => h.label)
        : applicationHeaders.map((header) => header.label);
    ...
    // Always include "Files" if not explicitly excluded
    if (
      !selectedFields ||
      selectedFields.length === 0 ||
      selectedFields.includes("files")
    ) {
      headers.push("Files");
    }

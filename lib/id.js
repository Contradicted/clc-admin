...
async function generateStudentID(campus, studyMode, retryCount = 0) {
  // Use Hybrid Learning campus code if study mode is hybrid learning
  if (studyMode && studyMode.toLowerCase() === "hybrid_learning") {
    campus = Campus.HYBRID;
  }
  ...
  const lastStudent = await db.enrolledStudent.findFirst({
    where: {
      AND: [{ campus: campus }, { id: { startsWith: prefix } }],
    },
    orderBy: {
      id: "desc",
    },
  ...
  if (existingId) {
    ...
    return generateStudentID(campus, studyMode, retryCount + 1);
  }

  return studentId;
}

async function createEnrolledStudent(applicationID, campus, studyMode) {
  ...
  return await db.$transaction(async (tx) => {
    ...
    if (
      ...
      campus !== Campus.BRISTOL &&
      campus !== Campus.SHEFFIELD &&
      campus !== Campus.BIRMINGHAM
    ) {
      throw new Error(
        "Invalid campus. Must be either London, Bristol, Sheffield or Birmingham"
      );
    }
    ...
    const studentId = await generateStudentID(campus, studyMode);
